...
* **NEW: Bleeding-Edge Nightly Builds** - CUDA 13.0 packages with Python 3.13, PyTorch nightly, and cutting-edge optimizations including FlashAttention, SageAttention, and NATTEN. See <<nightly-builds, Nightly Builds>> section.
...
This repository provides two build variants:

* *Stable Build (CUDA 12.8)*: Production-ready with tested PyTorch stable releases
** Package name: `ComfyUI_Windows_portable_cu128.7z.*`
** Uses PyTorch with CUDA 12.8 support
** Recommended for most users

* *Nightly Build (CUDA 13.0)*: Bleeding-edge with latest features
** Package name: `ComfyUI_Windows_portable_cu130_nightly.7z.*`
** Uses Python 3.13 + PyTorch 2.10+ nightly with CUDA 13.0
** Includes performance optimizations: FlashAttention, xformers, SageAttention, NATTEN, etc.
** Recommended for users who want the latest features and performance improvements

== How to Use

. Only NVIDIA GPUs are supported, preferably with the latest driver installed.
** For CUDA 13.0 nightly builds, ensure you have the latest NVIDIA drivers

. Download the package files from the
https://github.com/senopui/ComfyUI-Windows-Portable/releases[release page].
** For Stable Build (CUDA 12.8):
*** https://github.com/senopui/ComfyUI-Windows-Portable/releases/latest/download/ComfyUI_Windows_portable_cu128.7z.001[ComfyUI_Windows_portable_cu128.7z.001]
*** https://github.com/senopui/ComfyUI-Windows-Portable/releases/latest/download/ComfyUI_Windows_portable_cu128.7z.002[ComfyUI_Windows_portable_cu128.7z.002]
** For Nightly Build (CUDA 13.0):
*** https://github.com/senopui/ComfyUI-Windows-Portable/releases/download/nightly-cu130/ComfyUI_Windows_portable_cu130_nightly.7z.001[ComfyUI_Windows_portable_cu130_nightly.7z.001]
*** https://github.com/senopui/ComfyUI-Windows-Portable/releases/download/nightly-cu130/ComfyUI_Windows_portable_cu130_nightly.7z.002[ComfyUI_Windows_portable_cu130_nightly.7z.002]
** Models (shared between builds):
*** https://github.com/senopui/ComfyUI-Windows-Portable/releases/latest/download/models.zip.001[models.zip.001] (If you are upgrading from a previous release, you don't need this file.)
...
** *For Nightly Build (cu130)*: Two specialized launcher scripts are available:
*** `run_maximum_fidelity.bat`: Prioritizes quality and stability (disables xformers and smart memory)
*** `run_optimized_fidelity.bat`: Balances quality with performance (enables all optimizations)
** Both launchers maintain compatibility with `character_select_stand_alone_app_test` (default port 8188)

** *New in Nightly Builds:* Two additional launcher scripts are available:
*** `run_maximum_fidelity.bat` - Disables xformers and smart memory optimizations for maximum image quality. Use when you need the highest possible fidelity and don't mind slower performance.
*** `run_optimized_fidelity.bat` - Enables all bleeding-edge optimizations including auto precision, xformers/FlashAttention, and SageAttention. Best for balanced performance and quality.
...
Starting with CUDA 13.0, we provide bleeding-edge nightly builds that include the latest developments:

=== What's in Nightly Builds

* **Python 3.13** - Latest Python interpreter for improved performance
* **PyTorch Nightly** (2.10+) - Bleeding-edge PyTorch with latest optimizations
* **CUDA 13.0** - Latest CUDA toolkit support
* **Advanced Attention Mechanisms:**
** **FlashAttention** - Memory-efficient attention for faster inference
** **xformers** - Latest version from AI-windows-whl repository
** **SageAttention** - Advanced attention with Triton optimizations
** **NATTEN** - Neighborhood attention for improved quality
* **ComfyUI Master** - Latest ComfyUI from the master branch (no version pinning)
* **Latest Custom Nodes** - All nodes pulled from their respective master branches

=== Special Launcher Scripts

The nightly builds include two specialized launcher scripts in the `ExtraScripts` folder:

==== run_maximum_fidelity.bat
* Disables xformers and smart memory optimizations
* Prioritizes image quality over speed
* Use when: You need the absolute highest quality and consistency, and performance is not a concern
* Best for: Final production renders, quality comparisons, archival work

==== run_optimized_fidelity.bat
* Enables all bleeding-edge optimizations
* Features auto-precision, xformers, FlashAttention when available
* Use when: You want the best balance of speed and quality
* Best for: Interactive workflows, experimentation, faster iteration

=== Compatibility Notes

* **ComfyUI Default Port:** Nightly builds use the standard port 8188, ensuring compatibility with `character_select_stand_alone_app_test`
* **ASAR Setting:** The repository already has `ASAR=false` configured for compatibility
* **Stability:** Nightly builds include the latest features but may have occasional instabilities. For production work, consider using stable releases.

=== Downloading Nightly Builds

Nightly builds are available from the releases page with the tag `nightly-cu130`:

* https://github.com/senopui/ComfyUI-Windows-Portable/releases/tag/nightly-cu130[Nightly Build Releases]
