...
| CI: cu130 workflow run (URL, status) | NOT RUN — no `origin` remote or GitHub CLI/metadata available in this environment to dispatch or query runs. | NOT RUN |
...
## Key Findings
### Fixed items
- None verified in this environment (no CI logs or runtime artifacts available).
...
### Open Issues
- **Severity: High** — CI verification unavailable in this environment.
  - **Symptom:** cu130 / cu130-nightly workflow status and artifacts cannot be confirmed.
