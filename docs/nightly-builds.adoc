...
Optional accelerators are installed on a best-effort basis and **must not** fail the workflow when unavailable. Availability is recorded in `builder-cu130/accel_manifest.json` and copied into the portable tree at `ComfyUI/user/default/accel_manifest.json`. Launchers run `scripts/preflight_accel.py` to re-check availability and disable dependent custom nodes when required backends are missing.
