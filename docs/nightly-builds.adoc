...
The nightly build workflow provides bleeding-edge versions of ComfyUI with the latest Python, PyTorch, CUDA, and optimization libraries. These builds are intended for users who want to test the latest features and optimizations before they make it into stable releases. `cu130` remains the stable track; `cu130-nightly` is experimental and must never regress stable intent.
...
=== Advanced Optimization Libraries (Best-Effort)

Nightly builds *attempt* to install cutting-edge optimization libraries from curated wheel sources. Availability depends on upstream wheels for Python 3.13 + CUDA 13.0, so these are best-effort and explicitly gated when missing.

* **xformers** - Best-effort install attempt (no source builds)
* **FlashAttention** - Highly optimized attention implementation (PyPI binary-only first, wheels.json resolver with validated wheel URLs + fallback templates; gated when unavailable)
...
* **Nunchaku** - Optional optimizer backend (when available)
* **SpargeAttn** - Optional optimizer backend (when available)
...
1. **Stage 1: Gathering Dependencies**
   ...
   * Installs bleeding-edge packages via the wheels.json resolver (validated wheel URLs + fallback templates; no index URLs)
   * Installs standard dependencies from pak files
   * Fails fast if pip attempts to downgrade torch or pull CPU-only builds
   * Attempts optional VCS dependencies (`https://github.com/cozy-comfyui/cozy_comfy` and `https://github.com/cozy-comfyui/cozy_comfyui`) in best-effort mode and records results in `builder-cu130/vcs_optional_manifest.json`

2. **Stage 2: Assembling Repositories**
   * Clones ComfyUI from master branch
   * Clones all custom nodes
   * Downloads required models
   * Pre-packaging sanity checks for required launchers

3. **Validation Steps**
   * Validates packaged artifact layout before compression
   * Fails build if required launchers or core files are missing
...
The build is tested with:
----
timeout 120 ./python_standalone/python.exe -s ComfyUI/main.py --cpu --windows-standalone-build --quick-test-for-ci
----
...
If any traceback is detected, the build fails and logs are displayed. Evidence lives in GitHub Actions run logs under the Stage 1/2/Validation/Stage 3 steps and the uploaded manifest artifacts.

=== Optional Accelerator Policy

Optional accelerators are installed on a best-effort basis and **must not** fail the workflow when unavailable. Availability is recorded in `builder-cu130/accel_manifest.json` and copied into the portable tree at `ComfyUI/user/default/accel_manifest.json`. Launchers run `scripts/preflight_accel.py` to re-check availability and disable dependent custom nodes when required backends are missing. Stage 2 copies the repo-root `scripts/` directory into the portable root so the preflight entrypoint is always packaged; if it is missing, nightly builds warn and record the skip in `accel_manifest.json`.

Manifest entries capture the requested pattern/version, selected URL (if any), success, gate reason, and stderr excerpts to aid debugging.

NOTE: AI-windows-whl must be consumed via the wheels.json resolver and verified wheel URLs only. Do not reintroduce `-i/--index-url` or `/simple` index assumptions.

== Stability and Support

=== Stability Level

⚠️  **Experimental** - Nightly builds are less stable than tagged releases. Stable `cu130` remains the baseline for reliability.
...
# Validate YAML
python3 -c "import yaml; yaml.safe_load(open('.github/workflows/build-cu130-nightly.yml'))" # requires PyYAML; note as not run if unavailable
----
